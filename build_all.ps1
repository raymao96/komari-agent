...
foreach ($goos in $osList) {
    foreach ($goarch in $archList) {
        ...
        & go build -trimpath -ldflags "-X github.com/raymao96/komari-agent/update.CurrentVersion=$version -X github.com/raymao96/komari-agent/update.Repo=raymao96/komari-agent" -o "$outPath"
