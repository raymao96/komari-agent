...
for GOOS in "${OS_LIST[@]}"; do
  for GOARCH in "${ARCH_LIST[@]}"; do
    ...
    # 构建二进制文件
    env GOOS=$GOOS GOARCH=$GOARCH CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X github.com/raymao96/komari-agent/update.CurrentVersion=${VERSION} -X github.com/raymao96/komari-agent/update.Repo=raymao96/komari-agent" -o "./build/$BINARY_NAME"
